...
test.describe('using runInBrowser', () => {
...
test.describe('using mount', () => {
  test('forward properties to browser', async ({ page, mount }) => {
  ...
  test('expose callbacks to browser', async ({ page, mount }) => {
    let clickMessages: string[] = [];

    await mount(ButtonComponent, {
      on: {
        click: (message) => clickMessages.push(message),
      },
    });
