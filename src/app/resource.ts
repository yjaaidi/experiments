...
export function createResource<T>(fn: () => Observable<T>): Resource<T> {
  ...
  const internalSuspenser = inject(InternalSuspenser, { optional: true });
  ...
  internalSuspenser?.addResource(resource);
  destroyRef.onDestroy(() => internalSuspenser?.deleteResource(resource));
...
class InternalSuspenser {
  resources = signal<Resource<unknown>[]>([]);
...
class Suspenser {
  #internal = inject(InternalSuspenser);
  pending = computed(() => this.#internal.resources().some((r) => r.pending));
  success = computed(() =>
    this.#internal.resources().every((r) => r.status === 'success'),
  );
  errors = computed(() =>
    this.#internal
      .resources()
      .map((r) => (r.status === 'error' ? r.error : null))
      .filter((r) => r != null),
  );
}

export function provideSuspenser() {
  return [InternalSuspenser, Suspenser];
}

export function injectSuspenser() {
  return inject(Suspenser);
}
